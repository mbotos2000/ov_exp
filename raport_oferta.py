  ...
  with st.form('Inregistrare cerere'):
    ...
    if (st.session_state.step >= 4)&(option==optiuni[6])&(st.session_state['1_1']!=None):	
      ...
      for idx, row in enumerate(inputs):
        ...
        r_c3=str(row["c3"])
        r_c4=str(row["c4"])
        st.session_state.cap3i+=f"\n\t     3.{idx+1} Elaborare releveu {r_c1} - {r_c2} RON + TVA;\n"
        st.session_state.note+= f"\n\t     Termenul de elaborare pentru releveu {r_c1} este de maxim {r_c3} zile lucrătoare de la semnarea contractului și plata ratei 1, dar nu mai puțin de {r_c4} zile lucrătoare de la generarea norului de puncte.\n"
